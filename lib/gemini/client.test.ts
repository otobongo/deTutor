...
describe('Gemini client (GT-109)', () => {
  it('attaches the canonical system prompt to every call', async () => {
    ...
    const client = createGeminiClient(transport, 'test-model');
  ...
  it('injects scenario context as a layer, never replacing the prompt', async () => {
    ...
    const client = createGeminiClient(transport, 'test-model');
    await client.chat(messages, { context: 'You are a barista in a Berlin café.' });
  ...
  it('returns typed JSON when the schema validates', async () => {
    ...
    await expect(client.generateJson(messages, schema)).resolves.toEqual({ verdict: 'full' });
  ...
  it('retries malformed JSON exactly once, then fails with parse-failure', async () => {
    ...
    const failure = await client.generateJson(messages, schema).catch((error: unknown) => error);
  ...
  it('recovers when the retry returns valid JSON', async () => {
    ...
    const client = createGeminiClient(transport, 'test-model');
    const schema = z.object({ ok: z.boolean() });
    await expect(client.generateJson(messages, schema)).resolves.toEqual({ ok: true });
    expect(requests).toHaveLength(2);
  });
  ...
  it.each([
    ...
    const client = createGeminiClient(transport, 'test-model');
    const failure = await client.chat(messages).catch((error: unknown) => error);
    expect(failure).toBeInstanceOf(GeminiError);
